...
class Address extends \Opencart\System\Engine\Model {
    ...
    public function updateTaxAddress() {
        ...
        if (empty($this->session->data['payment_address']['customer_group_id'])) {
            $this->load->model('extension/ajax_quick_checkout/ajax_quick_checkout/account');
            $this->session->data['payment_address']['customer_group_id'] = $this->model_extension_ajax_quick_checkout_ajax_quick_checkout_account->getDefaultCustomerGroup();
        }
        $this->config->set('config_customer_group_id', (int)$this->session->data['payment_address']['customer_group_id']);

        $this->tax->setShippingAddress((int)$address['country_id'], (int)$address['zone_id']);
    ...
    public function getAddress($address_id) {
        ...
        if (VERSION > "4.0.1.1") {
            $address = $address_id ? $this->model_account_address->getAddress($this->customer->getId(), (int)$address_id) : [];
        } else {
            $address = $address_id ? $this->model_account_address->getAddress((int)$address_id) : [];
        }
