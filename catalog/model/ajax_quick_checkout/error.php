...
class Error extends \Opencart\System\Engine\Model {
    ...
    public function validateCheckout(){
        ...
        $steps = $this->model_extension_ajax_quick_checkout_ajax_quick_checkout_store->getReceivers();
        foreach($steps as $step){
            $this->load->controller('extension/ajax_quick_checkout/ajax_quick_checkout/'.$step.'|validate');
        }
      ...
      && !empty($state['config'][$state['session']['account']][$step]['fields'][$field_id]['errors'])
        ...
        foreach($errors as $error){
          foreach($error as $validate => $rule){
            ...
            //   $this->model_extension_ajax_quick_checkout_ajax_quick_checkout_store->updateState(array('errors', $step, $field_id), $error['text']);
    ...
    //   $this->model_extension_ajax_quick_checkout_ajax_quick_checkout_store->updateState(array('errors', $step, $field_id), '');
    ...
    public function false_google_recaptcha($rule, $value){
        ...
        if($rule == 0){
            ...
            if(stripos($this->request->server['REQUEST_URI'] ,'/index.php?route=extension/ajax_quick_checkout/confirm|update')){
                if(empty($value)){
