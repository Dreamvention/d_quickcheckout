 ...
 \ \   _  _\ \  \_|/_\ \   __  \ \  \       \ \  \ \ \  \\\  \ \   _  _\  
  \ \  \\  \\ \  \_|\ \ \  \ \  \ \  \____   \ \  \ \ \  \\\  \ \  \\  \| 
   \ \__\\ _\\ \_______\ \__\ \__\ \_______\  \ \__\ \ \_______\ \__\\ _\ 
    ...
    #Extensibility. 
    To extend the functionality of Ajax Quick Checkout you can simply add a controller to the ajax_quick_checkout
...
class Store extends \Opencart\System\Engine\Model {
    ...
    public function __construct($registry) {
        ...
        $this->load->model('extension/dv_opencart_patch/engine/load');
    }
    ...
    public function dispatch($action, $data) { 
        ...
        foreach($state['action'] as $receiver => $actions){
            if(in_array($action, $actions)){
                if (VERSION > "4.0.1.1") {
                    $this->load->controller('extension/ajax_quick_checkout/ajax_quick_checkout/'.$receiver.'.receiver', array( 'action' => $action, 'data' => $data));
                } else {
                    $this->load->controller('extension/ajax_quick_checkout/ajax_quick_checkout/'.$receiver.'|receiver', array( 'action' => $action, 'data' => $data));
                }
    ...
    public function initState(){
        $this->cache->delete('ajax_quick_checkout_state');
        ...
        if(!empty($settings['layout'])){
            ...
            $this->config->set('ajax_quick_checkout_layout', $settings['layout']);
        }else{
            $this->load->config('ajax_quick_checkout_layout/default');
            $state['layout'] = $this->config->get('ajax_quick_checkout_layout');
        }
    ...
    public function loadState(){
        if ($this->cache->get('ajax_quick_checkout_state')) {
            $state_cache = $this->cache->get('ajax_quick_checkout_state');
            $this->config->set('ajax_quick_checkout_config', $state_cache['config']);
            $this->config->set('ajax_quick_checkout_layout', $state_cache['layout']);
            $this->config->set('ajax_quick_checkout_errors', $state_cache['errors']);
            $this->config->set('ajax_quick_checkout_steps', $state_cache['steps']);
            $this->config->set('ajax_quick_checkout_action', $state_cache['action']);
            $this->config->set('ajax_quick_checkout_language', $state_cache['language']);
            $this->config->set('ajax_quick_checkout_notifications', array());
        } else {
            $this->initState();
            $this->model_extension_dv_opencart_patch_engine_load->controller('extension/ajax_quick_checkout/module/ajax_quick_checkout|initSteps');
            $state = $this->getState();
            $this->config->set('ajax_quick_checkout_config', $state['config']);
            $this->config->set('ajax_quick_checkout_layout', $state['layout']);
            $this->config->set('ajax_quick_checkout_errors', $state['errors']);
            $this->config->set('ajax_quick_checkout_steps', $state['steps']);
            $this->config->set('ajax_quick_checkout_action', $state['action']);
            $this->config->set('ajax_quick_checkout_language', $state['language']);
            $this->config->set('ajax_quick_checkout_notifications', array());
            $this->saveState();
        }
    ...
    public function setState($data, $save = false){
        ...
        $state_update = $this->config->get('ajax_quick_checkout_state_update');
        if(empty($state_update)){
            $state_update = array();
        }
        $state_update = array_replace_recursive($state_update, $data);
        $this->config->set('ajax_quick_checkout_state_update', $state_update);
        ...
        if(!empty($data)){
            foreach($data as $key => $state_new){
                ...
                $state_old = $this->config->get('ajax_quick_checkout_'.$key);
                if(is_array($state_old) && is_array($state_new)){
                    $state[$key] = array_replace_recursive($state_old, $state_new);
                }else{
                    $state[$key] = $state_new;
                }
                $this->config->set('ajax_quick_checkout_'.$key, $state[$key]);

                if($save){
                    $this->cache->set('ajax_quick_checkout_'.$key, $state[$key]);
                }
            }
        }
    }
    ...
    public function updateState($keys, $value) {
        ...
        if($keys[0] == 'session'){
            ...
            $this->config->set('ajax_quick_checkout_'.$keys[0], $state[$keys[0]]);
        }
    ...
    public function saveState(){
        $this->cache->set('ajax_quick_checkout_state', [
            'config' => $this->config->get('ajax_quick_checkout_config'),
            'layout' => $this->config->get('ajax_quick_checkout_layout'),
            'errors' => $this->config->get('ajax_quick_checkout_errors'),
            'steps' => $this->config->get('ajax_quick_checkout_steps'),
            'action' => $this->config->get('ajax_quick_checkout_action'),
            'language' => $this->config->get('ajax_quick_checkout_language'),
        ]);
    }
    ...
    public function getState($keys = ''){
        ...
        if(!empty($keys)){
            if(is_array($keys)){
                foreach($keys as $key){
                    if($key == 'session'){
                        ...
                        $state[$key] = ($this->config->get('ajax_quick_checkout_'.$key)) ? $this->config->get('ajax_quick_checkout_'.$key) : array();
                    }
                }
            }else{
                ...
                if($keys == 'session'){
                    ...
                    $state[$keys] = ($this->config->get('ajax_quick_checkout_'.$keys)) ? $this->config->get('ajax_quick_checkout_'.$keys) : array();
                }
            }
        }else{
            $state['session'] = $this->session->data;
            $state['layouts'] = ($this->config->get('ajax_quick_checkout_layouts')) ? $this->config->get('ajax_quick_checkout_layouts') : array();
            $state['skins'] = ($this->config->get('ajax_quick_checkout_skins')) ? $this->config->get('ajax_quick_checkout_skins') : array();
            $state['notifications'] = ($this->config->get('ajax_quick_checkout_notifications')) ? $this->config->get('ajax_quick_checkout_notifications') : array();
            $state['config'] = ($this->config->get('ajax_quick_checkout_config')) ? $this->config->get('ajax_quick_checkout_config') : array();
            $state['layout'] =($this->config->get('ajax_quick_checkout_layout')) ? $this->config->get('ajax_quick_checkout_layout') : array();
            $state['errors'] = ($this->config->get('ajax_quick_checkout_errors')) ? $this->config->get('ajax_quick_checkout_errors') : array();
            $state['steps'] = ($this->config->get('ajax_quick_checkout_steps')) ? $this->config->get('ajax_quick_checkout_steps') : array();
            $state['action'] = ($this->config->get('ajax_quick_checkout_action')) ? $this->config->get('ajax_quick_checkout_action') : array();
            $state['opencart_version'] = VERSION;
            $state['language_code'] = $this->config->get('config_language');
            ...
            $state['language_code'] = $this->config->get('config_language');
            $state['language'] = ($this->config->get('ajax_quick_checkout_language')) ? $this->config->get('ajax_quick_checkout_language') : array();
        }
    ...
    public function clearState(){
        ...
        $this->config->set('ajax_quick_checkout_config', array());
        $this->config->set('ajax_quick_checkout_layout', array());
        $this->config->set('ajax_quick_checkout_errors', array());
        $this->config->set('ajax_quick_checkout_steps', array());
        $this->config->set('ajax_quick_checkout_action', array());
        $this->config->set('ajax_quick_checkout_language', array());
        $this->config->set('ajax_quick_checkout_notifications', array());
        

        $store_id = $this->config->get('config_store_id');
        $code = 'ajax_quick_checkout'; 
        $this->clearSetting();
    }

    public function getStateUpdated(){
        $this->load->model('extension/ajax_quick_checkout/ajax_quick_checkout/order');
        $state['session']['order_id'] = $this->model_extension_ajax_quick_checkout_ajax_quick_checkout_order->updateOrder();
        $this->setState($state);

        $state = $this->config->get('ajax_quick_checkout_state_update');
    ...
    public function isUpdated($key){
        $updated = $this->config->get('ajax_quick_checkout_updated');
    ...
    public function setUpdated($data){
        ...
        $this->config->set('ajax_quick_checkout_updated', $updated);

    }

    public function getReceivers(){

        $dir = DIR_EXTENSION.'ajax_quick_checkout/catalog/controller/ajax_quick_checkout';
        $files = scandir($dir);
        $receivers = array();
    ...
    public function getLayouts(){
        $dir = DIR_EXTENSION.'ajax_quick_checkout' . $this->pro . '/system/config/ajax_quick_checkout_layout';
        $files = scandir($dir);
        $layouts = array();
        foreach($files as $file){
            if(strlen($file) > 1 && strpos( $file, '.php')){
    ...
    public function getSkins(){
        $dir = DIR_EXTENSION.'ajax_quick_checkout' . $this->pro . '/catalog/view/stylesheet/ajax_quick_checkout/skin';
        $files = scandir($dir);
        $layouts = array();
        foreach($files as $file){
    ...
    public function changeLayout($codename){
        ...
        $this->load->config('ajax_quick_checkout_layout/'.$codename);
        $value = $this->config->get('ajax_quick_checkout_layout');
        ...
        $state_cache = $this->cache->get('ajax_quick_checkout_state');
        if ($state_cache) {
            $state_cache['layout'] = $value;
            $this->cache->set('ajax_quick_checkout_state', $state_cache);
        }
    }
