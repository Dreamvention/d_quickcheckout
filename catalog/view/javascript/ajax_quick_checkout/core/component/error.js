...
(function() {
    this.validate = function(value, errors) {
        ...
        d_quickcheckout_lodash.mapValues(errors, function (error) {
            for (rule in error) {
                if (typeof that[rule] !== "undefined") {
                    if (!that[rule](error[rule], value)) {
                        result = error.text;
                    }
                }
            }
    ...
    this.goToError = function() {
        setTimeout(function() {
            if (this.getState()['captcha_status'] == 1 && this.in_array(this.getAccount(), this.getState()['config_captcha_page']) && this.getState()['captcha_type'] == 'google') {
                ...
                var sitekey = document.getElementById('input-payment-captcha').getAttribute('data-sitekey');
