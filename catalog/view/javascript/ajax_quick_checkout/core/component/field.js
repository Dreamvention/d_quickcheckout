...
(function() {
    this.initFieldSortable = function(step_id) {
        ...
        d_quickcheckout_sortable('#' + step_id + '_fields', {
            ...
            item.addEventListener('sortupdate', function (event) {
                var IDs = [];
                var els = document.getElementById(step_id + '_fields').getElementsByClassName("qc-field");

                for (el of els) {
                    IDs.push(el.getAttribute('field_id'));
                }

                var state = that.getState();
                for (var key in IDs) {
                    state.config['guest'][step_id].fields[IDs[key]].sort_order = parseInt(key);
                    state.config['register'][step_id].fields[IDs[key]].sort_order = parseInt(key);
                    state.config['logged'][step_id].fields[IDs[key]].sort_order = parseInt(key);
    ...
    this.getCustomField = function(data) {
        if (this.getState.opencart_version > "4.0.1.1") {
            var url = 'extension/ajax_quick_checkout/module/ajax_quick_checkout.get_custom_fields';
        } else {
            var url = 'extension/ajax_quick_checkout/module/ajax_quick_checkout|get_custom_fields';
        }
        this.send(url, data, function(custom_fields) {
    ...
    this.subscribe('field/addDepend', function(data) {
        ...
        if (typeof state.config[account][step_id].fields[field_id].depends == 'undefined') {
            state.config[account][step_id].fields[field_id].depends = {};
        }
        state.config[account][step_id].fields[field_id].depends[depend_id] = {};

        this.updateState(['config', account, step_id,'fields',field_id], state.config[account][step_id].fields[field_id]);
    ...
    this.subscribe('field/removeDepend', function(data) {
        ...
        if (typeof state.config[account][step_id].fields[field_id].depends !== 'undefined' && typeof state.config[account][step_id].fields[field_id].depends[depend_id] !== 'undefined') {
            delete state.config[account][step_id].fields[field_id].depends[depend_id];
            this.updateState(['config', account, step_id, 'fields', field_id, 'depends'], state.config[account][step_id].fields[field_id].depends);
        }
