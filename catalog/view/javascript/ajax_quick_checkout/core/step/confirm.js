...
(function() {
    this.subscribe('confirm/confirm', function(data) {
        ...
        if (version > '4.0.1.1') {
            var url = 'extension/ajax_quick_checkout/ajax_quick_checkout/confirm.update';
        } else {
            var url = 'extension/ajax_quick_checkout/ajax_quick_checkout/confirm|update';
        }
        setTimeout(function() {
            this.send(url, data, function(json) {
                ...
                if (!json.session.confirm.checkout) {
                    ...
                    this.loading(true);
                    setTimeout(function() {
                        var href = document.getElementById('payment')?.querySelector(json.session.confirm.trigger)?.getAttribute('href');
    ...
    this.subscribe('confirm/prev', function() {
        ...
        if (prev_page_id) {
            ...
            this.send('extension/ajax_quick_checkout/module/ajax_quick_checkout|update', state, function(json) {}.bind(this));
        }
    });
