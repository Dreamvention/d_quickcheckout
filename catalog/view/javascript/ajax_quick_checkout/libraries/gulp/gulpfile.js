...
gulp.task('browser-sync', function() {
    browserSync({
        proxy: 'http://localhost/opencart/302/ajax_quick_checkout/',
        files: [
            'stylesheet/ajax_quick_checkout/*.css',
        ]
    });
});
gulp.task('sass-core', function() {
    return gulp.src(path.join(themeDir, 'stylesheet/ajax_quick_checkout/main.s*ss'))
        .pipe(sass().on('error', sass.logError))
        .pipe(autoprefixer(['last 15 versions']))
        .pipe(gulp.dest(path.join(themeDir, 'stylesheet/ajax_quick_checkout/')))
        .pipe(browserSync.stream({ match: '**/*.css' }));

});
var list_tasks;

var style_folders = path.join(themeDir, 'stylesheet/ajax_quick_checkout/skin');
...
gulp.task('sass-rtl', function() {
    return gulp.src(path.join(themeDir, 'stylesheet/ajax_quick_checkout/rtl.s*ss'))
        .pipe(sass().on('error', sass.logError))
        .pipe(autoprefixer(['last 15 versions']))
        .pipe(gulp.dest(path.join(themeDir, 'stylesheet/ajax_quick_checkout/')))
        .pipe(browserSync.stream({ match: '**/*.css' }));
...
gulp.task('sass-default', function() {
    return gulp.src('stylesheet\\ajax_quick_checkout\\skin\\default\\default.s*ss')
        .pipe(sass().on('error', sass.logError))
        .pipe(autoprefixer({
            browsers: ["last 15 versions"]
        }))
        .pipe(gulp.dest('stylesheet\\ajax_quick_checkout\\skin\\default'))
        .pipe(browserSync.stream({ match: '**/*.css' }));
});
...
gulp.task('watch', ['browser-sync', 'sass-core', 'sass-rtl', 'sass_multi', 'core-scripts'], function() {
    gulp.watch(path.join(themeDir, 'stylesheet/ajax_quick_checkout/scss/**/*.s*ss'), ['sass-core']);
    gulp.watch(path.join(themeDir, 'stylesheet/ajax_quick_checkout/*.s*ss'), ['sass-rtl']);
    gulp.watch(path.join(themeDir, 'stylesheet/ajax_quick_checkout/skin/**/**/*.s*ss'), ['sass_multi']);
