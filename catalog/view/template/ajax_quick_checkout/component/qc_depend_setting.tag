<qc_depend_setting>
        ...
        <div
            ...
            <div class="ve-field">
                ...
                <div class="ve-input-group">
                    ...
                    type="text"
                    class="ve-input"
                    name="config[{opts.account}][{parent.opts.step}][fields][{ parent.opts.field_id }][depends][{parent.opts.depend_id}][{depend_value_id}][value]"
            ...
            <div class="row">
                <div class="ve-field col-md-6">
                    ...
                    <div>
                        <qc_switcher onclick="{parent.opts.edit}" name="config[{opts.account}][{parent.opts.step}][fields][{ parent.opts.field_id }][depends][{parent.opts.depend_id}][{depend_value_id}][display]" data-label-text="Enabled" value="{ depend_value.display }" />
                    </div>
                </div>
                ...
                <div class="ve-field col-md-6">
                    ...
                    <div>
                        <qc_switcher onclick="{parent.opts.edit}" name="config[{opts.account}][{parent.opts.step}][fields][{ parent.opts.field_id }][depends][{parent.opts.depend_id}][{depend_value_id}][require]" data-label-text="Enabled" value="{ depend_value.require }" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        getField(){
            return this.store.getState().config[tag.opts.account][tag.opts.step].fields[tag.opts.depend_id];
        }

        tag.field = this.getField();

        removeDepend(e){
            var depend_id = e.currentTarget.getAttribute('data-depend_id');
            tag.store.dispatch('field/removeDepend', {account: tag.opts.account, step_id : tag.opts.step, field_id: tag.opts.field_id , depend_id: depend_id } );
        }

        addDependValue(e){
            var depend_id = e.currentTarget.getAttribute('data-depend_id');
            tag.store.dispatch('field/addDependValue', {
                account: tag.opts.account,
        ...
        removeDependValue(e){
            var depend_id = e.currentTarget.getAttribute('data-depend_id');
            var depend_value_id = e.currentTarget.getAttribute('data-depend_value_id');
