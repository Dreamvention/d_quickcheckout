<qc_field_captcha>
    ...
    <qc_captcha_setting  if={riot.util.tags.selectTags().search('"qc_captcha_setting"') && getState().edit} field_id="{opts.field_id}" field="{opts.field}" step="{opts.step}" ondelete="{opts.ondelete}" ></qc_captcha_setting>

    <qc_pro_label if={ riot.util.tags.selectTags().search('"qc_captcha_setting"') < 0 && getState().edit}></qc_pro_label>
    ...
    <div if={ isVisible() && getState().captcha_status == 1 && in_array( getAccount(), getState().config_captcha_page ) && getState().captcha_type == 'basic'  } class="field-sortable d-vis ve-clearfix { (opts.error && isRequired() ) ? 've-field--error' : ''}">
        ...
        <div class="{ (getStyle() == 'list') ? 'col-half' : 'col-full'}">
			...
			<input
				...
				disabled=true
			/>
			<img src="index.php?route=extension/ajax_quick_checkout/ajax_quick_checkout_captcha/basic_captcha|captcha" alt="" />
