<qc_field_select>
    ...
    <div class="no-display" if={ (!isVisible() && getState().edit && typeof opts.field.display !== 'undefined') }>
     ...
     <script>
        ...
        getName(){
            var result = opts.field?.options?.filter ? opts.field.options.filter(function(item){
        ...
        this.on('mount', function(){
            if(tag.root.querySelector('.choiceselectpicker') && this.store.getState().config.guest[tag.opts.step].fields[tag.opts.field_id].search == 1){
                new Choices(tag.root.querySelector('.choiceselectpicker'));
            }
        })
