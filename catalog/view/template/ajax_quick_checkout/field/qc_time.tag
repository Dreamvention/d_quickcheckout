<qc_field_time>
    ...
    <div if={ isVisible() } class="field-sortable d-vis  ve-clearfix  { (opts.error && isRequired()) ? 've-field--error' : ''}">
        <label class="{ (getStyle() == 'list') ? 'col-half' : 'col-full'} ve-label" for="{ opts.step }_{ opts.field_id }">
            ...
            <span data-balloon-pos="up" aria-label="{ getLanguage()[parent.opts.step][opts.field.tooltip] }" if={ getLanguage()[opts.step][opts.field.tooltip] }><i class="fa fa-question-circle" ref="tooltip"></i></span>
        </label>
        ...
        <div class="{ (getStyle() == 'list') ? 'col-half' : 'col-full'}">
            <div if={!getState().edit } class="ve-input-group">
                ...
                <input
                    ...
                    class="ve-input d-vis { (opts.field.mask) ?  'qc-mask': '' } validate { isRequired() ? 'qc-required' : 'qc-not-required'} { opts.field.id }"
    ...
    <script>
        ...
        initTime(){    
            var locale = this.store.getState().language_code.split('-');
            locale = locale[0];        
            d_quickcheckout_flatpickr('#' + this.opts.step + '_' + this.opts.field_id, {
                ...
                onChange: function(selectedTime, dateTime, e) {
                    error = tag.store.validate(e.input.value, tag.opts.field.errors);
