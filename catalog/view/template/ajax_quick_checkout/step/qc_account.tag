<qc_account>
    ...
    <script>
        ...
        login(e){
            ...
            while ((element = element.parentNode) && element.nodeType !== 9) {
                if (element.nodeType === 1 && element.nodeName.toLowerCase() == 'form') {
                    formEls.push(element);
                }
            }
            this.store.dispatch('account/login', serializeJSON(formEls));
            e.preventDefault();
        }

        changeAccount(e){
            this.store.dispatch('account/update', { account: e.currentTarget.querySelector('input').value});
        }
        ...
        this.on('mount', function(){
            if (this.root.querySelector('#login_popup')) {
                document.getElementsByTagName('body')[0].insertAdjacentElement('beforeend', this.root.querySelector('#login_popup'));
            }
        })
