    ...
    <script>
        ...
        increase(e){
            ...
            while ((element = element.parentNode) && element.nodeType !== 9) {
                if (element.nodeType === 1 && element.classList && element.classList.contains('qc-quantity')) {
                    parentNodes.push(element);
                }
            }
            var $input = parentNodes[0].querySelector('input.qc-product-quantity');
            var product_id = e.currentTarget.getAttribute('data-product');
            var quantity = parseInt($input.value) + 1;
            data = {};

            var state = this.store.getState();
            
            state.session.cart.products.forEach(function(product, i) {
                if(product.key == product_id){
                    state.session.cart.products[i].quantity = quantity;
                }
            });
            ...
            data.cart = {};
            data.cart[$input.getAttribute('data-product')] = quantity;
        ...
        delete(e){
            ...
            var data = {};
            data.cart = {};
            data.cart[$input.getAttribute('data-product')] = 0;
        ...
        changeCoupon(e){
            var state = this.store.getState();
            if(e.currentTarget.value != state.session.coupon){
                Array.from(tag.root.querySelectorAll('.qc-coupon .ve-btn')).forEach(function(el) {
        ...
        useCoupon(e){
            ...
            var parentNodes = [];
            while ((element = element.parentNode) && element.nodeType !== 9) {
                if (element.nodeType === 1 && element.classList && element.classList.contains('qc-coupon')) {
                    parentNodes.push(element);
                }
            }
            var coupon = parentNodes[0].querySelector('input[name="coupon"]').value;
        ...
        changeVoucher(e){
            var state = this.store.getState();
            if(e.currentTarget.value != state.session.voucher){
                Array.from(tag.root.querySelectorAll('.qc-voucher .ve-btn--default')).forEach(function(el) {
                    el.classList.remove('ve-btn--default');
                    el.classList.add('ve-btn--primary');
                });
            }
        ...
        useVoucher(e){
            ...
            var parentNodes = [];
            while ((element = element.parentNode) && element.nodeType !== 9) {
                if (element.nodeType === 1 && element.classList && element.classList.contains('qc-voucher')) {
                    parentNodes.push(element);
                }
            }
            var voucher = parentNodes[0].querySelector('input[name="voucher"]').value;
            this.store.dispatch('cart/update_option', {voucher: voucher});
            Array.from(tag.root.querySelectorAll('.qc-voucher .ve-btn')).forEach(function(el) {
                    el.classList.remove('ve-btn--default');
                    el.classList.add('ve-btn--primary');
                });
        }
        ...
        changeReward(e){
            var state = this.store.getState();
            if(e.currentTarget.value != state.session.reward){
                Array.from(tag.root.querySelectorAll('.qc-reward .ve-btn--default')).forEach(function(el) {
                    el.classList.remove('ve-btn--default');
                    el.classList.add('ve-btn--primary');
                });
            }
        ...
        useReward(e){
            ...
            var parentNodes = [];
            while ((element = element.parentNode) && element.nodeType !== 9) {
                if (element.nodeType === 1 && element.classList && element.classList.contains('qc-reward')) {
                    parentNodes.push(element);
                }
            }
            var reward = parentNodes[0].querySelector('input[name="reward"]').value;
            this.store.dispatch('cart/update_option', {reward: reward});

            Array.from(tag.root.querySelectorAll('.qc-reward .ve-btn')).forEach(function(el) {
                    el.classList.remove('ve-btn--default');
                    el.classList.add('ve-btn--primary');
                });
        }
