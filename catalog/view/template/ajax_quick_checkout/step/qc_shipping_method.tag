<qc_shipping_method>
    <div class="step qc-shipping-method">
        ...
        <div class="ve-card" if={ getConfig().shipping_method.display == 1 && getState().config.guest.shipping_method.style == 'card' }>
            ...
            <div class="ve-card__section">
                ...
                <form id="shipping_method_list" if={getState().config.guest.shipping_method.display_options == 1 && getSession().shipping_methods}>
                    ...
                    class="radio-input" >
                        <div if={!d_quickcheckout_lodash.isEmpty(shipping_method?.quote)}>
                            ...
                            <div class="ve-field" each={ quote, index in shipping_method.quote } >
                                <label  for="{ quote.code }" class="ve-radio {(getSession().shipping_method == quote.code || getSession().shipping_method.code == quote.code) ? 've-radio--selected' : ''}">
                                    <input
                                    type="radio"
                                    class="ve-input"
                                    ...
                                    value="{ quote.code }"
                                    id="{ quote.code }"
                                    checked={ getSession().shipping_method == quote.code || getSession().shipping_method.code == quote.code }
                                    onclick={change}/>
                                    <i></i>
                                    <span class="text"><qc_raw content="{ quote.title || quote.name }"></qc_raw></span> <span class="price">{ quote.text }</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div if={getState().config.guest.shipping_method.input_style == 'select'}>
                        <select name="shipping_method" if={getState().config.guest.shipping_method.display_group_title == 1} class="ve-input" onchange={change}>
                            <optgroup if={!d_quickcheckout_lodash.isEmpty(shipping_method?.quote)} label="{ shipping_method.title || shipping_method.name }" 
                            each={ shipping_method, name in getSession().shipping_methods } >
                                <option 
                                    each={ quote, index in shipping_method.quote } 
                                    selected={ getSession().shipping_method == quote.code || getSession().shipping_method.code == quote.code }
                                    value="{ quote.code }">
                                    <span class="text"><qc_raw content="{ quote.title || quote.name }"></qc_raw></span> <span class="price">{ quote.text }</span>
                                </option>
                            </optgroup>
        ...
        <div class="ve-mb-3 ve-clearfix" if={ getConfig().shipping_method.display == 1 && getState().config.guest.shipping_method.style == 'clear' }>
            ...
            <form id="shipping_method_list" if={getState().config.guest.shipping_method.display_options == 1 && getSession().shipping_methods}>
                ...
                <div if={getState().config.guest.shipping_method.input_style == 'select'}>
                    <select name="shipping_method" if={getState().config.guest.shipping_method.display_group_title == 1} class="ve-input" onchange={change}>
                        <optgroup label="{ shipping_method.title || shipping_method.name }" 
                        each={ shipping_method, name in getSession().shipping_methods } >
                            <option 
                                each={ quote, index in shipping_method.quote } 
                                selected={ getSession().shipping_method == quote.code || getSession().shipping_method.code == quote.code }
    ...
    <script>
        ...
        change(e){
            this.store.dispatch('shipping_method/update', serializeJSON(e.currentTarget));
        }
